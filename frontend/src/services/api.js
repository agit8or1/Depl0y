...
export default {
  ...
  // Proxmox VM (direct PVE API proxy)
  pveVm: {
    config: (hostId, node, vmid) => api.get(`/pve-vm/${hostId}/${node}/${vmid}/config`),
    updateConfig: (hostId, node, vmid, data) => api.put(`/pve-vm/${hostId}/${node}/${vmid}/config`, data),
    status: (hostId, node, vmid) => api.get(`/pve-vm/${hostId}/${node}/${vmid}/status`),
    rrddata: (hostId, node, vmid, timeframe = 'hour') => api.get(`/pve-vm/${hostId}/${node}/${vmid}/rrddata`, { params: { timeframe } }),
    start: (hostId, node, vmid) => api.post(`/pve-vm/${hostId}/${node}/${vmid}/start`),
    stop: (hostId, node, vmid) => api.post(`/pve-vm/${hostId}/${node}/${vmid}/stop`),
    shutdown: (hostId, node, vmid) => api.post(`/pve-vm/${hostId}/${node}/${vmid}/shutdown`),
    reboot: (hostId, node, vmid) => api.post(`/pve-vm/${hostId}/${node}/${vmid}/reboot`),
    reset: (hostId, node, vmid) => api.post(`/pve-vm/${hostId}/${node}/${vmid}/reset`),
    suspend: (hostId, node, vmid) => api.post(`/pve-vm/${hostId}/${node}/${vmid}/suspend`),
    resume: (hostId, node, vmid) => api.post(`/pve-vm/${hostId}/${node}/${vmid}/resume`),
    delete: (hostId, node, vmid) => api.delete(`/pve-vm/${hostId}/${node}/${vmid}`),
    snapshots: (hostId, node, vmid) => api.get(`/pve-vm/${hostId}/${node}/${vmid}/snapshots`),
    createSnapshot: (hostId, node, vmid, data) => api.post(`/pve-vm/${hostId}/${node}/${vmid}/snapshots`, data),
    deleteSnapshot: (hostId, node, vmid, name) => api.delete(`/pve-vm/${hostId}/${node}/${vmid}/snapshots/${name}`),
    rollbackSnapshot: (hostId, node, vmid, name) => api.post(`/pve-vm/${hostId}/${node}/${vmid}/snapshots/${name}/rollback`),
    clone: (hostId, node, vmid, data) => api.post(`/pve-vm/${hostId}/${node}/${vmid}/clone`, data),
    migrate: (hostId, node, vmid, data) => api.post(`/pve-vm/${hostId}/${node}/${vmid}/migrate`, data),
    template: (hostId, node, vmid) => api.post(`/pve-vm/${hostId}/${node}/${vmid}/template`),
    addDisk: (hostId, node, vmid, data) => api.post(`/pve-vm/${hostId}/${node}/${vmid}/disk`, data),
    resizeDisk: (hostId, node, vmid, disk, data) => api.put(`/pve-vm/${hostId}/${node}/${vmid}/disk/${disk}/resize`, data),
    detachDisk: (hostId, node, vmid, disk, del = false) => api.delete(`/pve-vm/${hostId}/${node}/${vmid}/disk/${disk}`, { params: { delete: del } }),
    addNIC: (hostId, node, vmid, data) => api.post(`/pve-vm/${hostId}/${node}/${vmid}/network`, data),
    removeNIC: (hostId, node, vmid, nic) => api.delete(`/pve-vm/${hostId}/${node}/${vmid}/network/${nic}`),
    firewallRules: (hostId, node, vmid) => api.get(`/pve-vm/${hostId}/${node}/${vmid}/firewall/rules`),
    addFirewallRule: (hostId, node, vmid, data) => api.post(`/pve-vm/${hostId}/${node}/${vmid}/firewall/rules`, data),
    deleteFirewallRule: (hostId, node, vmid, pos) => api.delete(`/pve-vm/${hostId}/${node}/${vmid}/firewall/rules/${pos}`),
    updateFirewallRule: (hostId, node, vmid, pos, data) => api.put(`/pve-vm/${hostId}/${node}/${vmid}/firewall/rules/${pos}`, data),
    getFirewallOptions: (hostId, node, vmid) => api.get(`/pve-vm/${hostId}/${node}/${vmid}/firewall/options`),
    setFirewallOptions: (hostId, node, vmid, data) => api.put(`/pve-vm/${hostId}/${node}/${vmid}/firewall/options`, data),
    getVncTicket: (hostId, node, vmid) => api.post(`/pve-vm/${hostId}/${node}/${vmid}/vncticket`),
    // Aliases
  ...
  // Proxmox Node (direct PVE API proxy)
  pveNode: {
    clusterStatus: (hostId) => api.get(`/pve-node/${hostId}/cluster/status`),
    clusterResources: (hostId, type) => api.get(`/pve-node/${hostId}/cluster/resources`, { params: { type } }),
    nextId: (hostId) => api.get(`/pve-node/${hostId}/cluster/nextid`),
    nodeStatus: (hostId, node) => api.get(`/pve-node/${hostId}/nodes/${node}/status`),
    nodeRrddata: (hostId, node, timeframe = 'hour') => api.get(`/pve-node/${hostId}/nodes/${node}/rrddata`, { params: { timeframe } }),
    nodeVms: (hostId, node) => api.get(`/pve-node/${hostId}/nodes/${node}/vms`),
    tasks: (hostId, node, params) => api.get(`/pve-node/${hostId}/nodes/${node}/tasks`, { params }),
    taskLog: (hostId, node, upid) => api.get(`/pve-node/${hostId}/nodes/${node}/tasks/${encodeURIComponent(upid)}/log`),
    storage: (hostId, node) => api.get(`/pve-node/${hostId}/nodes/${node}/storage`),
    storageContent: (hostId, node, storage, content) => api.get(`/pve-node/${hostId}/nodes/${node}/storage/${storage}/content`, { params: { content } }),
    deleteVolume: (hostId, node, storage, volid) => api.delete(`/pve-node/${hostId}/nodes/${node}/storage/${storage}/content/${volid}`),
    network: (hostId, node) => api.get(`/pve-node/${hostId}/nodes/${node}/network`),
    applyNetwork: (hostId, node) => api.put(`/pve-node/${hostId}/nodes/${node}/network`),
    haResources: (hostId) => api.get(`/pve-node/${hostId}/cluster/ha/resources`),
    haGroups: (hostId) => api.get(`/pve-node/${hostId}/cluster/ha/groups`),
    haStatus: (hostId) => api.get(`/pve-node/${hostId}/cluster/ha/status`),
    pools: (hostId) => api.get(`/pve-node/${hostId}/pools`),
    pveUsers: (hostId) => api.get(`/pve-node/${hostId}/access/users`),
    containers: (hostId, node) => api.get(`/pve-node/${hostId}/nodes/${node}/lxc`),
    ctStatus: (hostId, node, vmid) => api.get(`/pve-node/${hostId}/nodes/${node}/lxc/${vmid}/status`),
    ctAction: (hostId, node, vmid, action) => api.post(`/pve-node/${hostId}/nodes/${node}/lxc/${vmid}/${action}`),
    ctConfig: (hostId, node, vmid) => api.get(`/pve-node/${hostId}/nodes/${node}/lxc/${vmid}/config`),
    ctSnapshots: (hostId, node, vmid) => api.get(`/pve-node/${hostId}/nodes/${node}/lxc/${vmid}/snapshots`),
    vzdump: (hostId, node, data) => api.post(`/pve-node/${hostId}/nodes/${node}/vzdump`, data),
    backupSchedules: (hostId) => api.get(`/pve-node/${hostId}/cluster/backup`),
    // Aliases used by StorageBrowser
    listStorage: (hostId, node) => api.get(`/pve-node/${hostId}/nodes/${node}/storage`),
    browseStorage: (hostId, node, storage, params) => api.get(`/pve-node/${hostId}/nodes/${node}/storage/${storage}/content`, { params }),
  },
  ...
  pveNode: {
    ...
    deleteHaResource: (h, sid) => api.delete(`/pve-node/${h}/cluster/ha/resources/${sid}`),
    ...
    createUserToken: (h, uid, tid) => api.post(`/pve-node/${h}/access/users/${uid}/tokens/${tid}`),
