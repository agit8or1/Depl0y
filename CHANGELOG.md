...
### Added
- **`docs/SCREENSHOTS.md` — a 27-view screenshot gallery**, 13 dark and 14 light, captured at 1440×1000 at 2× device scale from the running application with the panel's own Settings → Appearance selector rather than injected CSS. Table of contents, five workflow sections, and per image a heading, theme label, route, caption, alt text and full-size link. Generated from the capture manifest so captions cannot drift.
- **Walkthrough video** attached to the v2.2.75 release: a 3½-minute 1080p30 H.264 recording with burned-in captions, a 55-second highlight cut, a poster frame and SRT/VTT caption files. Linked from the README via a clickable poster, since GitHub renders neither `<video>` nor iframes in Markdown.
